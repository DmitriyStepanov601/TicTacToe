...
public class AI {
	...
	public Move miniMax(Board board) {
        if (playerLetter == Constants.X) {   
            return max(new Board(board), 0);  // If the X plays then it wants to MAXimize the heuristics value
        } else {
            return min(new Board(board), 0);  // If the O plays then it wants to MINimize the heuristics value
        }
	}
